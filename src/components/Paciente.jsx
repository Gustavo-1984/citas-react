...
const Paciente = ({paciente, setPaciente, eliminarPaciente}) => {
  ...
  const handleEliminar = () =>{
    const res = confirm('Deseas eliminar paciente')
    if(res){
      eliminarPaciente(id)
    }
  }
  ...
  return (
    ...
    <div className='mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl'>
        ...
        <button onClick={() => setPaciente(paciente)} className='py-2 px-10 bg-indigo-600 hover:bg-indigo-700 rounded text-white font-bold uppercase'>Editar</button>
        <button onClick={handleEliminar} className='py-2 px-10 bg-red-600 hover:bg-red-700 rounded text-white font-bold uppercase'>Eliminar</button>
    </div>
    </div>
